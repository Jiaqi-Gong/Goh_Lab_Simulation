...
class DomainGenerator:
    ...
    def _generateOctagon(self, surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int],
        ...
        # Find the center of the hexagon
        # If the length is an odd number, the center of the octagon should be located between 4 points (ie center point should end as .5)
        if cen[0] % 2 == cen[1] % 2 and domainWidth % 2 == 1:
            if cen[0] % 2 == 1:
                cen = [cen[0], cen[1]]
            elif cen[0] % 2 == 0:
                cen = [cen[0] - 0.5, cen[1] - 0.5]
        elif cen[0] % 2 != cen[1] % 2 and domainWidth % 2 == 1:
            if cen[0] % 2 == 1:
                cen = [cen[0], cen[1] - 0.5]
            elif cen[1] % 2 == 1:
                cen = [cen[0] - 0.5, cen[1]]
        ...
        # create large square
        for i in range(int(cen[0] - (domainWidth / 2) - domainWidth), int(cen[0] + (domainWidth / 2) + domainWidth + 1)):
            for j in range(int(cen[1] - (domainWidth / 2) - domainWidth), int(cen[1] + (domainWidth / 2) + domainWidth + 1)):
                if surface[i, j] == 0:
                    # Initialize charge
                    charge = self._generatePositiveNegative(charge_concentration)
                    surface[i, j] = charge
        ...
        # bottom left edge
        bl = (int(cen[0] + domainWidth / 2 + domainWidth), int(cen[1] - domainWidth / 2 - domainWidth))
        eg_bl = domainWidth
        for i in range(0, domainWidth):
            for j in range(0, eg_bl):
                # remove the edge triangle
                if surface[int(bl[0] - i), int(bl[1] + j)] == 1:
                    surface[int(bl[0] - i), int(bl[1] + j)] = 0
                    List[0] -= 1
                elif surface[int(bl[0] - i), int(bl[1] + j)] == -1:
                    surface[int(bl[0] - i), int(bl[1] + j)] = 0
                    List[1] -= 1
            eg_bl -= 1

        # bottom right edge
        br = (int(cen[0] + domainWidth / 2 + domainWidth), int(cen[1] + domainWidth / 2 + domainWidth))
        eg_br = domainWidth
        for i in range(0, domainWidth):
            for j in range(0, eg_br):
                # remove the edge triangle
                if surface[int(br[0] - i), int(br[1] - j)] == 1:
                    surface[int(br[0] - i), int(br[1] - j)] = 0
                    List[0] -= 1
                elif surface[int(br[0] - i), int(br[1] - j)] == -1:
                    surface[int(br[0] - i), int(br[1] - j)] = 0
                    List[1] -= 1
            eg_br -= 1

            # top right edge
        tr = (int(cen[0] - domainWidth / 2 - domainWidth), int(cen[1] + domainWidth / 2 + domainWidth))
        eg_tr = domainWidth
        for i in range(0, domainWidth):
            for j in range(0, eg_tr):
                # remove the edge triangle
                if surface[int(tr[0] + i), int(tr[1] - j)] == 1:
                    surface[int(tr[0] + i), int(tr[1] - j)] = 0
                    List[0] -= 1
                elif surface[int(tr[0] + i), int(tr[1] - j)] == -1:
                    surface[int(tr[0] + i), int(tr[1] - j)] = 0
                    List[1] -= 1
            eg_tr -= 1

            # top left edge
        tl = (int(cen[0] - domainWidth / 2 - domainWidth), int(cen[1] - domainWidth / 2 - domainWidth))
        eg_tl = domainWidth
        for i in range(0, domainWidth):
            for j in range(0, eg_tl):
                # remove the edge triangle
                if surface[int(tl[0] + i), int(tl[1] + j)] == 1:
                    surface[int(tl[0] + i), int(tl[1] + j)] = 0
                    List[0] -= 1
                elif surface[int(tl[0] + i), int(tl[1] + j)] == -1:
                    surface[int(tl[0] + i), int(tl[1] + j)] = 0
                    List[1] -= 1
            eg_tl -= 1
