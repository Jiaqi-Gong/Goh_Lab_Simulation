...
def nearestPoint(surface: Surface, point: List[Union[int, int, int]]) -> List[Union[int, int, int]]:
    """
...
def _diamondEmpty(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
    ...
    if startPoint[1] == 0 or startPoint[1] == int(surface.shape[1]) - 1:
        for i in range(0, ln + 1):
            for j in range(0, eg):
                ...
                # top right
                point = nearestPoint(surface, [int(startPoint[2] - i), startPoint[1], int(startPoint[0] + j)])
    ...
    if startPoint[2] == 0 or startPoint[2] == int(surface.shape[0]) - 1:
        for i in range(0, ln + 1):
            for j in range(0, eg):
                ...
                # top right
                point = nearestPoint(surface, [startPoint[2], int(startPoint[1] - i), int(startPoint[0] + j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
    ...
    if startPoint[0] == 0 or startPoint[0] == int(surface.shape[2]) - 1:
        for i in range(0, ln + 1):
            for j in range(0, eg):
                ...
                # top right
                point = nearestPoint(surface, [int(startPoint[2] - i), int(startPoint[1] + j), startPoint[0]])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
...
def _generateDiamond(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                     charge: int) -> ndarray:
    """
...
def _crossEmpty(surface: Surface, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                possible_charge: List[int]) -> bool:
    """
    ...
    if startPoint[0] == 0 or startPoint[0] == int(surface.shape[2]) - 1:
        ...
        for i in range(domainWidth + 1):
            # bottom line
            point = nearestPoint(surface, [int(startPoint[2] + i), int(startPoint[1]), startPoint[0]])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
        ...
        for j in range(domainLength + 1):
            # right line
            point = nearestPoint(surface, [int(startPoint[2]), int(startPoint[1] + j), startPoint[0]])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
    ...
    if startPoint[1] == 0 or startPoint[1] == int(surface.shape[1]) - 1:
        ...
        for i in range(domainWidth + 1):
            # bottom line
            point = nearestPoint(surface, [int(startPoint[2] + i), startPoint[1], int(startPoint[0])])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
        ...
        for j in range(domainLength + 1):
            # right line
            point = nearestPoint(surface, [int(startPoint[2]), startPoint[1], int(startPoint[0] + j)])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
    ...
    if startPoint[2] == 0 or startPoint[2] == int(surface.shape[0]) - 1:
        ...
        for i in range(domainWidth + 1):
            # bottom line
            point = nearestPoint(surface, [startPoint[2], int(startPoint[1] + i), int(startPoint[0])])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
        ...
        for j in range(domainLength + 1):
            # right line
            point = nearestPoint(surface, [startPoint[2], int(startPoint[1]), int(startPoint[0] + j)])
            if surface[point[0], point[1], point[2]] in possible_charge:
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                return False
...
def _generateCross(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                   charge: int) -> ndarray:
    """
...
def _octagonEmpty(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                  possible_charge: List[int]) -> bool:
    """
    ...
    if startPoint[0] == 0 or startPoint[0] == int(surface.shape[2]) - 1:
        ...
        if ln % 2 == 1:
            ...
            for i in range(n):
                for j in range(n):
                    # 1st point
                    point = nearestPoint(surface, [int(cen[2] + (0.5 + i)), int(cen[1] - (0.5 + j)), cen[0]])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd point
        ...
        # If the length is even
            ...
            for i in range(n + 1):
                for j in range(n + 1):
                    # 1st points
                    point = nearestPoint(surface, [int(cen[2] + i), int(cen[1] + j), cen[0]])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd points
        ...
        for i in range(0, ln + 1):
            for j in range(0, eg):
                # top right
                point = nearestPoint(surface, [int(ed_tr[0] - i), int(ed_tr[1] + j), ed_tr[2]])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # top left
                point = nearestPoint(surface, [int(ed_tl[0] - i), int(ed_tl[1] - j), ed_tl[2]])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom right
                point = nearestPoint(surface, [int(ed_br[0] + i), int(ed_br[1] + j), ed_br[2]])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom left
                point = nearestPoint(surface, [int(ed_bl[0] + i), int(ed_bl[1] - j), ed_bl[2]])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
    ...
    if startPoint[1] == 0 or startPoint[1] == surface.shape[1] - 1:
        ...
        if ln % 2 == 1:
            ...
            for i in range(n):
                for j in range(n):
                    # 1st point
                    point = nearestPoint(surface, [int(cen[2] + (0.5 + i)), cen[1], int(cen[0] - (0.5 + j))])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd point
        ...
        # If the length is even
            ...
            for i in range(n + 1):
                for j in range(n + 1):
                    # 1st point
                    point = nearestPoint(surface, [int(cen[2] + i), cen[1], int(cen[0] + j)])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd point
        ...
        for i in range(0, ln + 1):
            for j in range(0, eg):
                # top right
                point = nearestPoint(surface, [int(ed_tr[0] - i), ed_tr[1], int(ed_tr[2] + j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # top left
                point = nearestPoint(surface, [int(ed_tl[0] - i), ed_tl[1], int(ed_tl[2] - j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom right
                point = nearestPoint(surface, [int(ed_br[0] + i), ed_br[1], int(ed_br[2] + j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom left
                point = nearestPoint(surface, [int(ed_bl[0] + i), ed_bl[1], int(ed_bl[2] - j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
    ...
    if startPoint[2] == 0 or startPoint[2] == surface.shape[0] - 1:
        ...
        if ln % 2 == 1:
            ...
            for i in range(n):
                for j in range(n):
                    # 1st point
                    point = nearestPoint(surface, [cen[2], int(cen[1] + (0.5 + i)), int(cen[0] - (0.5 + j))])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd point
        ...
        # If the length is even
            ...
            for i in range(n + 1):
                for j in range(n + 1):
                    # 1st point
                    point = nearestPoint(surface, [cen[2], int(cen[1] + i), int(cen[0] + j)])
                    # if the point is on a domain charge, location is not empty and need to choose new starting point
                    if surface[point[0], point[1], point[2]] in possible_charge:
                        return False
                    # 2nd point
        ...
        for i in range(0, ln + 1):
            for j in range(0, eg):
                # top right
                point = nearestPoint(surface, [ed_tr[0], int(ed_tr[1] - i), int(ed_tr[2] + j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # top left
                point = nearestPoint(surface, [ed_tl[0], int(ed_tl[1] - i), int(ed_tl[2] - j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom right
                point = nearestPoint(surface, [ed_br[0], int(ed_br[1] + i), int(ed_br[2] + j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
                # bottom left
                point = nearestPoint(surface, [ed_bl[0], int(ed_bl[1] + i), int(ed_bl[2] - j)])
                # if the point is on a domain charge, location is not empty and need to choose new starting point
                if surface[point[0], point[1], point[2]] in possible_charge:
                    return False
...
def _generateOctagon(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                     charge: int) -> ndarray:
    """
...
def _singleEmpty(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                 possible_charge: List[int]) -> bool:
    """
    This function check the position want to generate single is empty
    """
    # locate the closest valid point
    point = nearestPoint(surface, [startPoint[2], startPoint[1], startPoint[0]])
    # if the point is on a domain charge, location is not empty and need to choose new starting point
    if surface[point[0], point[1], point[2]] in possible_charge:
        return False
...
def _generateSingle(surface: ndarray, domainWidth: int, domainLength: int, startPoint: Tuple[int, int, int],
                    charge: int) -> ndarray:
    """
